...
> [!TLDR]
> 사용자는 회사 지식을 활용하여 ChatGPT를 맞춤화하기 위해 OpenAI의 개방형 프로토콜인 모델 컨텍스트 프로토콜(MCP) 기반의 원격 서버를 직접 구축하고 ChatGPT에 연결할 수 있으며, 이 과정에서 안전 및 보안 고려사항을 반드시 이해해야 합니다.
>
> - MCP 서버는 `검색` 및 `가져오기` 도구를 통해 ChatGPT의 심층 연구 기능을 확장합니다.
> - 서버 구축 시 OAuth 및 동적 클라이언트 등록을 통한 인증, 그리고 인트라넷 환경에서는 터널링(예: ngrok)이 필수적입니다.
> - 신뢰할 수 없는 MCP 서버 연결은 숨겨진 지침(프롬프트 인젝션)이나 악의적인 동작을 유발할 수 있으므로, 연결 전 철저한 실사와 데이터 공유 위험 검토가 중요합니다.
...
5.  선택적으로 (ChatGPT Enterprise, Edu 또는 Team 관리자의 경우), 커넥터를 전체 작업 공간에 게시합니다. 그러면 심층 연구에서 추가 지음 소스로 나타납니다.
...
MCP 서버를 테스트하려면, API [플레이그라운드](https://platform.openai.com/playground)를 사용하여 서버에 접근 가능한지, 도구 목록이 예상대로 해결되는지 확인하세요. 플레이그라운드를 사용하여 심층 연구를 실행하기 전에 서버가 결과를 반환하는 능력을 부분적으로 확인할 수 있습니다. 심층 연구는 반복적인 개선 (예: 검색 도구 설명을 개선하는 동안)에 더 시간이 오래 걸립니다.
...
사용자 지정 MCP 서버를 사용하면 ChatGPT 작업 공간을 외부 애플리케이션에 연결할 수 있으며, 이를 통해 ChatGPT는 이러한 애플리케이션에서 데이터에 접근하고, 데이터를 송수신하며, 조치를 취할 수 있습니다. 사용자 지정 MCP 서버는 OpenAI에서 개발하거나 검증한 것이 아니며, 자체 이용 약관이 적용되는 타사 서비스입니다.

악의적인 MCP 서버를 발견하면 [security@openai.com](mailto:security@openai.com)으로 신고해 주십시오.
### 신뢰할 수 있는 서버 연결

ChatGPT 작업 공간에 추가하는 사용자 지정 MCP 서버에 주의하십시오. 현재 ChatGPT에서는 사용자 지정 MCP 서버를 통한 심층 연구만 지원하며, 이는 원격 MCP 서버 내에서 사용 가능한 도구가 **검색** 및 **문서 검색**으로 제한됨을 의미합니다. 그러나 이처럼 범위가 좁더라도 위험은 여전히 존재합니다.

기반 애플리케이션을 알고 신뢰하지 않는 한 사용자 지정 MCP 서버에 연결하지 않는 것을 권장합니다. 예를 들어, 서비스 제공업체 자체에서 호스팅하는 공식 서버를 선택하십시오 (예: 제3자가 호스팅하는 사용자 지정 Stripe MCP 서버 대신 mcp.stripe.com에서 Stripe 자체에서 호스팅하는 Stripe 서버에 연결하는 것을 권장합니다). 현재 공식 원격 MCP 서버가 많지 않으므로, 해당 서버를 운영하지 않고 단순히 API를 통해 해당 서비스로 요청を 프록시하는 조직에서 호스팅하는 MCP 서버를 사용하고 싶을 수 있습니다. 이렇게 할 경우, 이러한 비공식 MCP 서버에 대해 실사를 철저히 수행하고, 데이터 사용 방식에 대해 신중하게 검토하고 서버를 신뢰할 수 있음을 확인한 후에만 연결하십시오. 자체 MCP 서버를 구축하고 연결할 때는 올바른 서버인지 다시 한번 확인하십시오.

악의적인 MCP 서버에는 ChatGPT가 예기치 않게 동작하도록 설계된 숨겨진 지침(프롬프트 인젝션)이 포함될 수 있습니다. OpenAI는 이러한 위협을 감지하고 차단하는 데 도움이 되는 내장 보호 장치를 구현했지만, 신뢰할 수 있는 서버하고만 연결이 설정되었는지 신중하게 검토하고 확인하는 것이 중요합니다.

자체 도구 정의를 하는 MCP 서버에 연결할 때, 귀하의 조직은 해당 MCP 서버 호스트와 공유하고 싶지 않거나 공유할 의도가 없는 데이터에 대한 요청을 받을 수 있습니다. 어떤 MCP 서버에든 연결하기 전에 공유될 데이터 유형을 신중하고 철저하게 검토하십시오.
MCP 서버는 도구 동작을 예기치 않게 업데이트하여 의도치 않거나 악의적인 동작을 초래할 수 있습니다.

### 서버 구축

접근을 허용하는 데이터에 주의하십시오. 귀하의 원격 MCP 서버는 다른 사람들이 OpenAI를 귀하의 서비스에 연결할 수 있도록 허용하며, OpenAI가 이러한 서비스에서 데이터에 접근하고, 데이터를 송수신하며, 조치를 취할 수 있도록 합니다. 도구의 JSON에 민감한 정보를 넣지 마십시오. 또한 원격 MCP 서버에 접근하는 ChatGPT 사용자로부터 민감한 정보를 저장하지 마십시오.
